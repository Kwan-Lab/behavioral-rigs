...
v.FramesPerTrigger = 10*900; % 10 frames per second = Records for up
% to 30 seconds for each trial

% DAQ detection start
lh = addlistener(s,'DataAvailable', @stopWhenExceedOneV);
sh = addlistener(s, 'DataAvailable', @saveWhenExceedOneV);
...
for trialNum=1:maxTrial
    ...
    trigger(v); % pause(0.01);
    if tt.save == 1
        tic;
        fname = [filename,'_',num2str(trialNum-1,'%.3d'),'.mat'];
        save(fname, 'data','TriggerDelayTime', 'SaveDelayTime', 'lhDelay');
        disp('data saved');
        tt.save = 0;
